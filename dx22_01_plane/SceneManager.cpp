...
float SceneManager::GetLoadProgress() const
{
	...
	if (!m_NextSceneId) return 0.0f; // ← 追加

	auto it = m_Scenes.find(*m_NextSceneId); // ← 修正
...
{
	auto it = find_or_end(m_Scenes, m_CurrentSceneId); // ← 修正
	return (it != m_Scenes.end()) ? it->second.scene.get() : nullptr;
}

const IScene* SceneManager::GetCurrentScene() const
{
	auto it = m_CurrentSceneId ? m_Scenes.find(*m_CurrentSceneId) : m_Scenes.end(); // ← 修正
	return (it != m_Scenes.end()) ? it->second.scene.get() : nullptr;
}
