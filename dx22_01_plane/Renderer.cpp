...
void Renderer::Init()
{
	...
	//鏡用のレンダーターゲットビュー
	ID3D11Texture2D* renderTargetMirror{};
	hr = m_SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&renderTargetMirror);
	if (renderTargetMirror != nullptr)m_Device->CreateRenderTargetView(renderTargetMirror, NULL, &m_RenderTargetView_Mirror);
	renderTargetMirror->Release();

	// デプスステンシルバッファ作成
	ID3D11Texture2D* depthStencile{};
	D3D11_TEXTURE2D_DESC textureDesc{};//レンダーテクスチャ
	...
	//鏡用のレンダーテクスチャ
	D3D11_TEXTURE2D_DESC textureDescMirror{};//レンダーテクスチャ
	textureDescMirror.Width = swapChainDesc.BufferDesc.Width;   // バッファの幅をスワップチェーンに合わせる
	textureDescMirror.Height = swapChainDesc.BufferDesc.Height; // バッファの高さをスワップチェーンに合わせる
	textureDescMirror.MipLevels = 1;                            // ミップレベルは1（ミップマップは使用しない）
	textureDescMirror.ArraySize = 1;                            // テクスチャの配列サイズ（通常1）
	textureDescMirror.Format = DXGI_FORMAT_D16_UNORM;           // フォーマットは16ビットの深度バッファを使用
	textureDescMirror.SampleDesc = swapChainDesc.SampleDesc;    // スワップチェーンと同じサンプル設定
	textureDescMirror.Usage = D3D11_USAGE_DEFAULT;              // 使用方法はデフォルト（GPUで使用）
	textureDescMirror.BindFlags = D3D11_BIND_DEPTH_STENCIL;     // 深度ステンシルバッファとして使用
	textureDescMirror.CPUAccessFlags = 0;                       // CPUからのアクセスは不要
	textureDescMirror.MiscFlags = 0;                            // その他のフラグは設定なし
	...
	light.Diffuse = Color(1.0f, 1.0f, 0.1f, 1.0f);
