...
private:
	...
	SootRainEffect* m_SootEffect = nullptr;//煤エフェクト

	std::weak_ptr<TitleEnemy> m_TitleEnemy;
...
//========================
	...
	std::vector<std::weak_ptr<SootObject>> m_MenuObjects;
	int m_SelectedIndex = 0;// 現在選択中のメニュー項目インデックス

	std::weak_ptr<SootObject> m_CursorObject;
